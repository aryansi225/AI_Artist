...
#Argparser creation to take command like argument for taking in the 
# --> Base Video Path --> Reference Image Path --> Prefix for the saved results --> Output Image Size
# --> Weight of content --> Weight of the Style --> Scale factor for weight of style --> Total variations in the Weights
# --> Number of iterartions --> Rescale factor for the image --> Rescale image algorithm --> Maintain the aspect ratio of image or not
# --> Content layer --> Part of initial image to be used(Content/Noise)
...
parser = argparse.ArgumentParser("Video transformer using Deep Convolutional Neural Network in Keras")
parser.add_argument('--base_video_path', metavar='base', type=str, help='Path to the video to transform.') #Base Image Path
...
#Converts string to boolean if user enters true in any of the boolean taking arguments like rescale or maintain aspect ratio
def strToBool(value):
...
# use weight given in arguments in the
...
# evaluate the loss and gradient descent
...
#get total loss combining all the losses
...
## Part 3 -- Code to extract frame from base video
...
vidcap = cv2.VideoCapture(base_video_path)
success,image = vidcap.read()
count = 0
fps = vidcap.get(cv2.cv.CV_CAP_PROP_FPS)
#codec = vidcap.get(cv2.cv.CV_CAP_PROP_FOURCC)
codec = cv2.cv.CV_FOURCC(*'XVID')
fwidth = vidcap.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH)
fheight = vidcap.get(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT)
while success:
    success,image = vidcap.read()
...
## Part 4 -- Main Code for producing processed frame and adding to make video
...
count1 = 0;
out = cv2.VideoWriter('output.avi', int(codec), fps, (int(fwidth), int(fheight)))
while count1<count:

    base_image_path = 'frame%d.jpg'% count1
